    ...
    <div className="p-6">
      ...
      <div className="mb-4">
        <input
          ...
          className="w-full max-w-sm rounded-xl border border-clinic-border px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-clinic-primary"
        />
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-clinic-border bg-white shadow-clinic">
        <table className="w-full text-sm">
          <thead className="border-b border-clinic-border bg-clinic-bg">
            <tr>
              <th className="px-4 py-3 text-left font-semibold text-clinic-text">Tên dịch vụ</th>
              <th className="px-4 py-3 text-left font-semibold text-clinic-text">Loại</th>
              <th className="px-4 py-3 text-right font-semibold text-clinic-text">Giá</th>
              <th className="px-4 py-3 text-center font-semibold text-clinic-text">Trạng thái</th>
              {canEdit && <th className="px-4 py-3 text-center font-semibold text-clinic-text">Thao tác</th>}
            </tr>
          </thead>
          <tbody>
            {isLoading ? (
              ...
              filtered.map((svc) => (
                <tr key={svc.id} className="border-b border-clinic-border last:border-0 hover:bg-clinic-bg/50">
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${svc.isActive ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-400'}`}>
                  ...
                  {canEdit && (
                    <td className="px-4 py-3 text-center">
                      <button
                        ...
                        className={`rounded-lg px-3 py-1 text-xs font-medium ${svc.isActive ? 'bg-red-50 text-red-600 hover:bg-red-100' : 'bg-green-50 text-green-700 hover:bg-green-100'} disabled:opacity-50`}
                      >
