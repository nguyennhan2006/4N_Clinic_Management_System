...
Nhóm áp dụng quy trình phát triển kết hợp theo hướng plan-driven theo phase: chốt yêu cầu và thiết kế trước, sau đó hiện thực và kiểm thử theo từng phase ngắn. Với mỗi module, nhóm xác định yêu cầu, thiết kế, hiện thực trên branch riêng, review, tích hợp, kiểm thử và chốt baseline trước khi mở rộng sang chức năng tiếp theo. Cách tổ chức này giúp nhóm kiểm soát mối liên hệ giữa yêu cầu, thiết kế, hiện thực và kiểm thử, đồng thời giảm rủi ro do phạm vi mở rộng quá sớm.
...
\hline
Backend & 21 feature folders, 20 controllers có API route, 24 service files (21 nghiệp vụ + 3 hạ tầng) \\
\hline
API & 92 endpoints, gồm 41 endpoints Phase 1 và 51 endpoints Phase 2 \\
\hline
Frontend & 36 named routes, 33 page files, tổ chức theo feature \\
\hline
Testing & 7 e2e test files (220/220 pass, theo log nhóm), 4 unit test files hộp trắng (59/59 pass, đếm trực tiếp từ mã nguồn); còn thiếu e2e cho inventory và pharmacy \\
\hline
Deployment & Local development only; CI scaffold (\code{.github/workflows/ci.yml}) đã thêm, chưa có Docker/production \\
\hline
\end{tabularx}
\end{table}
...
\hline
\textbf{Bước} & \textbf{Actor} & \textbf{Chức năng demo} & \textbf{Minh chứng } \\
\hline
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.9\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/screenshots/dashboard-admin.png}
        \caption{Dashboard của Admin}
    \end{subfigure}
    ...
    \begin{subfigure}{0.9\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/screenshots/dashboard-doctor.png}
        \caption{Dashboard của Bác sĩ}
    \end{subfigure}

    \caption{Minh chứng giao diện tổng quan theo vai trò người dùng}
    \label{fig:intro-dashboard-demo}
\end{figure}